...
uint32_t kRepresentationFormat = SL_ANDROID_PCM_REPRESENTATION_SIGNED_INT;
uint16_t kBitsPerSample = SL_PCMSAMPLEFORMAT_FIXED_16;
...
Java_com_google_sample_echo_MainActivity_createSLEngine(
    ...
    if (engine.soundTouch_) {
        ...
        engine.soundTouch_->setTempoChange(0.5);
        engine.soundTouch_->setPitchSemiTones(0.5);
        engine.soundTouch_->setRateChange(0.5);
...
Java_com_google_sample_echo_MainActivity_createSLBufferQueueAudioPlayer(JNIEnv *env, jclass type) {
    ...
    // sampleFormat.representation_ = kRepresentationFormat;
            ...
            if (engine.filterOn_ ) {
              ...
              do {
                  count = engine.soundTouch_->receiveSamples(
                          &samples[count], samplesCount - count);
              } while (count == 0);
              buf->size_ = count * 2;
            }
            break;
...
extern "C"
JNIEXPORT void JNICALL
Java_com_google_sample_echo_MainActivity_enableFilter(JNIEnv *env, jclass type, jboolean enable) {
  engine.filterOn_ = enable ? true : false;
