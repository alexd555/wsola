...
void AudioRecorder::ProcessSLCallback(SLAndroidSimpleBufferQueueItf bq) {
    ...
    callback_(ctx_, ENGINE_SERVICE_MSG_NEW_AUDIO_FRAME, dataBuf);
    ...
    if(devShadowQueue_->size() == 0) {
        LOGE("=====There is no buffer from device: %d", devShadowQueue_->size());
        LOGE("=====FreeBufferQueue: %d", freeQueue_->size());
